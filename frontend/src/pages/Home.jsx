...
const Home = ({ showCustomModal, isAuthenticated }) => {
  ...
  const displayRecommendations = (fromLocation = '', toLocation = '') => {
    ...
    // Add general tips if not enough location-specific recommendations
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      if (activeSearchTab === 'bus') {
        ...
        params = { location: from, checkIn: date }; // Hotel uses 'location' for 'from'
      }

      const response = await axios.get(endpoint, { params });
      setSearchResults(response.data);
      setNoResults(response.data.length === 0);
      displayRecommendations(from, to); // Update recommendations based on search
    } catch (error) {
      console.error('Search error:', error);
      showCustomModal('Error', error.response?.data?.message || 'Failed to fetch listings. Please try again.');
      setNoResults(true); // Assume no results on error too
      setSearchResults([]); // Clear any partial results
  ...
  const showDetail = (item, type) => {
    ...
    setSelectedBusSeat(null); // Reset seat selection
    setPassengerName('');
    setLeadGuestName('');
    setNumGuests(1);
  };
  ...
  const handleBookNow = async () => {
    ...
    try {
      if (selectedItem.type === 'bus') {
        ...
        const checkinDate = selectedItem.checkIn; // These are from hotel object
        const checkoutDate = selectedItem.checkOut;
        const start = new Date(checkinDate);
        const end = new Date(checkoutDate);
        const diffTime = Math.abs(end - start);
        const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        itemPrice = selectedItem.pricePerNight * Math.max(1, diffDays); // Ensure at least 1 day for calculation
        ...
        // Explicit confirmation for hotel booking with calculated price
      ...
      // Check wallet balance (frontend check, backend will also validate)
...
const renderBusSeats = (totalSeats, filledSeats) => {
  ...
  }, [activeSearchTab]); // Rerun search when tab changes
        ...
        <section id="search-results-section" className="container mx-auto px-4 py-8 mb-20">
                  ...
                  >
                    {activeSearchTab === 'bus' ? (
                      <>
                        ...
                        />
                        <h3 className="text-xl font-bold mb-2 text-blue-800 flex items-center"><span className="material-icons mr-2">directions_bus</span>{item.operator} - {item.busType}</h3>
          ...
          {!loading && showDetailView && selectedItem && (
            <div id="detail-view" className="bg-white p-6 rounded-xl shadow-lg border border-gray-100">
                ...
                {(selectedItem.galleryImages && selectedItem.galleryImages.length > 0) ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
                    {selectedItem.galleryImages.map((imgUrl, index) => (
                      <img
                        key={index}
                        src={getFullImageUrl(imgUrl)} // Use getFullImageUrl here
                  ...
                  <img
                    src={getFullImageUrl(selectedItem.type === 'bus' ? selectedItem.mainImageUrl : selectedItem.imageUrl) || 'https://placehold.co/400x250/CCCCCC/666666?text=No+Images+Available'} // Use getFullImageUrl here
              ...
              >
                {loading ? 'Confirming...' : (isAuthenticated ? 'Confirm Booking' : 'Login to Book')}
              </button>
            </div>
          )}
